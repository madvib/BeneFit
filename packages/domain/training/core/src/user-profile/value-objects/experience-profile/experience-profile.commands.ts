...
// Update functions
export function updateExperienceLevel(
...
export function addPreviousProgram(
  profile: ExperienceProfile,
  program: string,
): ExperienceProfile {
...
// Query functions
export function hasExperienceIn(
  profile: ExperienceProfile,
  programType: string,
): boolean {
  return profile.history.previousPrograms.some((p) =>
    p.toLowerCase().includes(programType.toLowerCase()),
  );
}

export function canPerform(
  profile: ExperienceProfile,
  activity: keyof CurrentCapabilities,
): boolean {
