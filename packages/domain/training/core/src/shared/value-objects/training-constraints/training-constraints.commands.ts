...
export function isInjurySeverity(
  ...
  const injury = constraints.injuries.find(
...
export function canExercise(
  ...
  for (const injury of constraints.injuries) {
    if (
      injury.avoidExercises.some((avoid) =>
        avoid.toLowerCase().includes(exercise.toLowerCase()),
      )
    ) {
      return false;
    }
...
export function canTrainAtTime(
  constraints: TrainingConstraints,
  time: string,
): boolean {
...
export function getDaysUntilNextAvailable(
  constraints: TrainingConstraints,
  fromDay: string,
): number {
...
export function withAddedEquipment(
  constraints: TrainingConstraints,
  equipment: string[],
): TrainingConstraints {
...
export function withAddedAvailableDay(
  constraints: TrainingConstraints,
  day: string,
): TrainingConstraints {
...
export function withAddedInjury(
  constraints: TrainingConstraints,
  injury: Injury,
): TrainingConstraints {
  const newInjuries = constraints.injuries
    ? [...constraints.injuries, injury]
    : [injury];
  return {
    ...constraints,
...
export function withExtendedDuration(
  constraints: TrainingConstraints,
...
export function withNewLocation(
  constraints: TrainingConstraints,
  location: string,
): TrainingConstraints {
...
export function withoutInjury(
  constraints: TrainingConstraints,
  bodyPart: string,
): TrainingConstraints {
  if (!constraints.injuries) return constraints;

  const newInjuries = constraints.injuries.filter(
    (injury) => injury.bodyPart.toLowerCase() !== bodyPart.toLowerCase(),
  );
...
// Calculation methods
export function getWeeklyTrainingAvailability(
  constraints: TrainingConstraints,
): number {
...
export function getEquipmentAvailabilityScore(
  constraints: TrainingConstraints,
...
export function getConstraintSummary(constraints: TrainingConstraints): string {
  ...
  if (hasInjuries(constraints)) {
    parts.push(
      `${constraints.injuries!.length} injury${constraints.injuries!.length !== 1 ? 'ies' : ''}`,
    );
  }
...
export function equals(
  ...
  return (
    ...
    JSON.stringify(constraints.availableEquipment) ===
      JSON.stringify(other.availableEquipment) &&
