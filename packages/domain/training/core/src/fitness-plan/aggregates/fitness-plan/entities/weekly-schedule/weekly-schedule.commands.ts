...
export function addWorkout(
  ...
  if (workout.planId !== schedule.planId) {
    return Result.fail(
      new ScheduleConflictError('Workout does not belong to this plan', {
        workoutPlanId: workout.planId,
        schedulePlanId: schedule.planId,
      }),
    );
  }
...
export function removeWorkout(
  ...
  if (index === -1) {
    return Result.fail(
      new WorkoutNotFoundError('Workout not found in this week', {
        workoutId,
  ...
  if (workout?.status === 'completed') {
    return Result.fail(
      new ScheduleModificationError('Cannot remove completed workouts', {
        workoutId,
