...
describe('BlogCategory', () => {
  describe('create', () => {
    ...
    it('should fail when value is empty string', () => {
      ...
      if (result.isFailure) {
        expect(result.errorMessage).toBe(
          'Category value is required and must be a string',
        );
      }
    });
    ...
    it('should fail when value is only whitespace', () => {
      ...
      expect(result.isFailure).toBe(true);
      if (result.isFailure) {
        expect(result.errorMessage).toBe('Category value cannot be empty');
      }
    });
    ...
    it('should fail when value exceeds 50 characters', () => {
      ...
      expect(result.isFailure).toBe(true);
      if (result.isFailure) {
        expect(result.errorMessage).toBe('Category value cannot exceed 50 characters');
      }
    });
    ...
    it('should fail when value contains invalid characters', () => {
      ...
      expect(result.isFailure).toBe(true);
      if (result.isFailure) {
        expect(result.errorMessage).toBe('Category value contains invalid characters');
      }
    });
