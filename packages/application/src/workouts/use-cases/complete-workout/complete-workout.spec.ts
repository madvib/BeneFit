...
import * as profileDomain from '@bene/core/profile';
...
describe('CompleteWorkoutUseCase', () => {
  ...
  it('should complete a workout successfully', async () => {
    ...
    vi.mocked(plansDomain.WorkoutPlanCommands.completeWorkout).mockReturnValue(Result.ok({ id: 'plan-1' } as any));

    const mockProfile = {
      id: 'user-1',
      stats: { totalWorkouts: 5, totalVolume: 1000, totalMinutes: 100, currentStreak: 1 },
    };
    ...
    vi.mocked(profileDomain.UserProfileCommands.recordWorkoutCompleted).mockReturnValue({
      ...mockProfile,
      stats: { totalWorkouts: 6, totalVolume: 1000, totalMinutes: 145, currentStreak: 2 },
    } as any);

    vi.mocked(workoutsDomain.createCompletedWorkout).mockReturnValue(Result.ok({ id: 'completed-1' } as any));
