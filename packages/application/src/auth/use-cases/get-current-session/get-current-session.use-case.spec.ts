...
describe('GetCurrentSessionUseCase', () => {
  ...
  it('should return a successful result when executing', async () => {
    ...
    mockAuthService.getSession.mockResolvedValue(Result.ok({ id: '1', email: 'test@example.com' }));
    mockAuthService.getCurrentUser.mockResolvedValue(Result.ok({ id: '1', email: 'test@example.com', name: 'Test User' }));
