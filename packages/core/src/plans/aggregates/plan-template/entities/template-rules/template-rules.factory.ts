...
import { ExperienceLevel, TemplateRestrictions, CustomizableParameter, TemplateRules } from "./template-rules.js";
...
export function createTemplateRules(props: {
  ...
  if (props.requiredDaysPerWeek !== undefined) {
    const daysResult = Guard.inRange(props.requiredDaysPerWeek, 1, 7, 'requiredDaysPerWeek');
  ...
  if (props.restrictions) {
    if (props.restrictions.minSessionMinutes !== undefined) {
      const minResult = Guard.againstNegativeOrZero(props.restrictions.minSessionMinutes, 'restrictions.minSessionMinutes');
      if (minResult.isFailure) return Result.fail(minResult.error);
    }
    if (props.restrictions.maxSessionMinutes !== undefined) {
      const maxResult = Guard.againstNegativeOrZero(props.restrictions.maxSessionMinutes, 'restrictions.maxSessionMinutes');
  ...
  if (props.customizableParameters) {
    for (const param of props.customizableParameters) {
      ...
      if (param.type === 'select') {
        const optionsGuard = Guard.againstNullOrUndefined(param.options, 'parameter.options for select type');
        if (optionsGuard.isFailure) return Result.fail(optionsGuard.error);

        const lenGuard = Guard.isTrue(param.options!.length > 0, 'select parameter must have options');
