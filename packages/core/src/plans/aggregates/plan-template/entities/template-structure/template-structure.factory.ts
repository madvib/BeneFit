...
import { TemplateDuration, TemplateFrequency, WeekTemplate, TemplateStructure } from "./template-structure.js";
...
export function createTemplateStructure(props: {
  ...
  if (props.duration.type === 'fixed') {
    const durationResult = Guard.againstNegativeOrZero(props.duration.weeks, 'duration.weeks');
  ...
  if (props.frequency.type === 'fixed') {
    const freqResult = Guard.inRange(props.frequency.workoutsPerWeek, 1, 7, 'frequency.workoutsPerWeek');
  ...
  const weeksLenGuard = Guard.isTrue(props.weeks.length > 0, 'weeks array cannot be empty');
  ...
  for (const week of props.weeks) {
    ...
    const workoutLenResult = Guard.isTrue(week.workouts.length > 0, `week ${ week.weekNumber } must have at least one workout`);
    ...
    for (const workout of week.workouts) {
      ...
      if (typeof workout.durationMinutes === 'number') {
        const durResult = Guard.againstNegativeOrZero(workout.durationMinutes, 'workout.durationMinutes');
        if (durResult.isFailure) return Result.fail(durResult.error);
      }
      const actLenResult = Guard.isTrue(workout.activities.length > 0, 'workout must have at least one activity');
