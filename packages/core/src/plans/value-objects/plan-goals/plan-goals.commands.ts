...
export function hasTargetWeights(goals: PlanGoals): boolean {
  return goals.targetMetrics.targetWeights !== undefined && goals.targetMetrics.targetWeights.length > 0;
}
...
// Exercise queries
export function getTargetWeightForExercise(goals: PlanGoals, exercise: string): number | null {
  if (!goals.targetMetrics.targetWeights) {
    return null;
  }

  const targetWeight = goals.targetMetrics.targetWeights.find(w => w.exercise.toLowerCase() === exercise.toLowerCase());
...
export function withNewTargetDate(goals: PlanGoals, newDate: Date): Result<PlanGoals> {
  if (newDate <= new Date()) {
    return Result.fail(new GoalsValidationError('Target date must be in the future', { newDate }));
  }
...
export function withExtendedTargetDate(goals: PlanGoals, additionalWeeks: number): Result<PlanGoals> {
  if (!goals.targetDate) {
    return Result.fail(new GoalsValidationError('Cannot extend target date: no current target date', { additionalWeeks }));
  }
...
export function withUpdatedPrimaryGoal(goals: PlanGoals, newPrimary: string): Result<PlanGoals> {
...
export function withAdditionalSecondaryGoal(goals: PlanGoals, newSecondary: string): Result<PlanGoals> {
...
export function withUpdatedTargetPace(goals: PlanGoals, newPace: number): Result<PlanGoals> {
  if (newPace <= 0) {
    return Result.fail(new GoalsValidationError('Target pace must be positive', { newPace }));
  }
...
export function withUpdatedTargetWeight(goals: PlanGoals, exercise: string, newWeight: number): Result<PlanGoals> {
  ...
  let updatedWeights: TargetWeight[];
  if (goals.targetMetrics.targetWeights) {
    // Find if exercise already exists and update it
    const existingIndex = goals.targetMetrics.targetWeights.findIndex(w => w.exercise.toLowerCase() === exercise.toLowerCase());
    if (existingIndex !== -1) {
      ...
      updatedWeights = [...goals.targetMetrics.targetWeights, { exercise, weight: newWeight }];
    }
  } else {
...
export function withAdjustedTargetMetrics(goals: PlanGoals, factor: number): Result<PlanGoals> {
