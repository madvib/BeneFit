  ...
  if (props.intervals) {
    for (const interval of props.intervals) {
      if (interval.duration <= 0) {
        return Result.fail(
          new StructureValidationError('Interval duration must be positive', { duration: interval.duration })
        );
      }
      if (interval.rest < 0) {
        return Result.fail(
          new StructureValidationError('Interval rest cannot be negative', { rest: interval.rest })
        );
      }
    }
  }
