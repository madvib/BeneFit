...
Examples:
  npm run db:studio:do TrainingSession
  npm run db:studio:do TrainingSession user-123
  npm run db:list:do TrainingSession
  `);
  process.exit(1);
}
...
if (!dbPath || !existsSync(dbPath)) {
  console.error(`❌ Database not found for ${className}${instanceId ? ` (${instanceId})` : ''}\n`);
