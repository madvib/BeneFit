...
export const integrationSyncLog = sqliteTable(
  'integration_sync_log',
  {
    ...
    syncStartedAt: integer('sync_started_at', { mode: 'number' }).notNull(),
    syncCompletedAt: integer('sync_completed_at', { mode: 'number' }),
  ...
  (table) => [
    index('integration_sync_log_user_id_idx').on(table.userId),
  ]
);
