...
export const checkIns = sqliteTable(
  ...
  (table) => ({
    conversationIdIdx: index('check_ins_conversation_id_idx').on(table.conversationId),
    statusIdx: index('check_ins_status_idx').on(table.status),
    conversationStatusIdx: index('check_ins_conversation_status_idx').on(table.conversationId, table.status),
  })
);
