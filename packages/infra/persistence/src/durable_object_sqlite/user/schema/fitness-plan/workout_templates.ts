...
export const workoutTemplates = sqliteTable(
  ...
  (table) => ({
    planIdIdx: index('workout_templates_plan_id_idx').on(table.planId),
    weekIdIdx: index('workout_templates_week_id_idx').on(table.weekId),
    statusIdx: index('workout_templates_status_idx').on(table.status),
    scheduledDateIdx: index('workout_templates_scheduled_date_idx').on(table.scheduledDate),
    planWeekDayIdx: index('workout_templates_plan_week_day_idx').on(table.planId, table.weekNumber, table.dayOfWeek),
  })
);
