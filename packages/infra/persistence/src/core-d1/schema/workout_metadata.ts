...
export const workoutMetadata = sqliteTable(
  ...
  (table) => ({
    userIdCompletedAtIdx: index('workout_metadata_user_id_completed_at_idx').on(table.userId, table.completedAt),
    teamIdCompletedAtIdx: index('workout_metadata_team_id_completed_at_idx').on(table.teamId, table.completedAt),
    completedAtIdx: index('workout_metadata_completed_at_idx').on(table.completedAt),
  })
);
