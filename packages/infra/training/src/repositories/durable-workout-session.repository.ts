...
import { type DOClient, workout_session_schema, sessionMetadata as sessionMetadataTable } from '@bene/persistence';
import {
  QueryError,
  SaveError,
  DeleteError,
...
export class DurableWorkoutSessionRepository implements WorkoutSessionRepository {
  ...
  async findActiveByUserId(userId: string): Promise<Result<WorkoutSession>> {
    try {
      ...
      if (!result) {
        return Result.fail(new EntityNotFoundError('WorkoutSession', `active session for user ${ userId }`));
      }
  ...
  async save(session: WorkoutSession): Promise<Result<void>> {
    try {
      ...
      await this.db.insert(sessionMetadataTable).values(row).onConflictDoUpdate({
  ...
  async delete(sessionId: string): Promise<Result<void>> {
    try {
      await this.db.delete(sessionMetadataTable).where(eq(sessionMetadataTable.id, sessionId));
