...
import {
  ...
  achievementToDomain,
} from '../../src/mappers/user-profile.mapper.js';
import { profile, userStats, achievements } from '../../src/data/schema';
import { user_do_schema } from '../../src/data/schema/index.js';
import migrations from '../../migrations/migrations.js';

/**
 * Tests for UserProfile Mapper using Cloudflare Durable Objects
 */
describe('UserProfileMapper (Durable Objects)', () => {
  ...
  describe('toProfileDatabase', () => {
    it('should map domain UserProfile to profile schema', async () => {
      await runInDurableObject(stub, async (instance: any, state: any) => {
        ...
        expect(dbProfile.lastAssessmentDate).toEqual(
          userProfile.experienceProfile.lastAssessmentDate,
        );
        expect(dbProfile.fitnessGoalsTargetDate).toEqual(userProfile.fitnessGoals.targetDate);

        // JSON fields - dates removed
        const { lastAssessmentDate: _lastAssessmentDate, ...experienceWithoutDate } =
          userProfile.experienceProfile;
        const { targetDate: _fitnessTargetDate, ...fitnessGoalsWithoutDate } =
  ...
  describe('toStatsDatabase', () => {
    it('should map domain UserStats to stats schema', async () => {
      await runInDurableObject(stub, async (instance: any, state: any) => {
        const userProfile = createUserProfileFixture({ userId: crypto.randomUUID() });
  ...
  describe('achievementToDatabase & achievementToDomain', () => {
    it('should map achievement bidirectionally', async () => {
      await runInDurableObject(stub, async (instance: any, state: any) => {
