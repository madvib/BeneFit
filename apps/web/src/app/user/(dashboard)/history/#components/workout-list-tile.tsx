...
import { getActivityColorClass, getActivityIcon } from '../../#shared/activity-styles';
...
export default function HistoryRow(workout: WorkoutData) {
  return (
    <tr key={workout.id} className="group hover:bg-muted/20 transition-colors">
      ...
      <td className="text-muted-foreground px-6 py-4 font-mono text-xs">{workout.date}</td>
      <td className="px-6 py-4">
        {workout.duration && (
      ...
      <td className="px-6 py-4">
        {workout.status && (
          <Badge variant={mapStatusToBadgeVariant(workout.status)}>{workout.status}</Badge>
        )}
      </td>
