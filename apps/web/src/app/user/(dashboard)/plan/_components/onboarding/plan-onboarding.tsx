...
export function PlanOnboarding({ onGenerate, onBrowse, isLoading }: Readonly<PlanOnboardingProps>) {
  const [showGoalForm, setShowGoalForm] = useState(false);

  if (showGoalForm) {
    return (
      <div className="animate-in fade-in slide-in-from-bottom-4 container mx-auto max-w-5xl p-6 py-12 duration-500">
      ...
      <div className="grid w-full max-w-3xl grid-cols-1 gap-6 md:grid-cols-2">
        ...
        <div className="group relative overflow-hidden rounded-3xl border border-blue-500/20 bg-gradient-to-br from-blue-500/5 to-purple-500/5 p-8 transition-all hover:border-blue-500/40 hover:shadow-xl">
          ...
          <Button
            ...
            isLoading={isLoading}
            className={`${typography.p} flex w-full items-center justify-center gap-2 rounded-2xl py-7 transition-transform active:scale-95`}
          >
        ...
        <div className="group border-border bg-background hover:border-muted-foreground/30 relative overflow-hidden rounded-3xl border p-8 transition-all hover:shadow-lg">
          ...
          <Button
            onClick={onBrowse}
            variant="outline"
            className={`${typography.p} flex w-full items-center justify-center gap-2 rounded-2xl py-7`}
          >
