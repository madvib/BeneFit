...
export default function AchievementPopup({
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-md transition-opacity duration-500 sm:p-6 ${isVisible ? 'opacity-100' : 'opacity-0'}`}
      onClick={onClose}
    >
      <div
        className={`bg-background/90 ring-border/50 relative w-full max-w-md transform overflow-hidden rounded-[32px] p-6 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.5)] ring-1 transition-all duration-700 ease-out sm:p-10 ${isVisible ? 'translate-y-0 scale-100' : 'translate-y-20 scale-90 rotate-2'}`}
