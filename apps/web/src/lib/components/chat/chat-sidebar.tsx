...
export function ChatSidebar({
  ...
  return (
    <>
      ...
      {/* Sidebar Content */}
      <aside
        className={`bg-accent/30 border-muted fixed inset-y-0 left-0 z-40 flex h-full w-[280px] transform flex-col border-r transition-transform duration-300 ease-in-out md:static md:transform-none ${isOpen ? "translate-x-0" : "-translate-x-full md:-ml-[280px]"} ${className || ''}`}
      >
        {/* Header Area */}
         {/* onClose is always required so this area always renders if header or implicit close button is present */}
         <div className="flex items-center justify-between p-4 pb-2">
                 {header}
                 <Button variant="ghost" size="sm" onClick={onClose} className="md:hidden ml-auto">
                    <X size={18} />
                 </Button>
            </div>
